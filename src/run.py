...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", args.map_name, "{}".format(args.experiment), "tb_logs", args.mixer)
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.save_model and (runner.t_env - model_save_time >= args.save_model_interval or model_save_time == 0):
            ...
            save_path = os.path.join(args.local_results_path, args.map_name, "{}".format(args.experiment), "models", args.mixer, args.unique_token, str(runner.t_env))
